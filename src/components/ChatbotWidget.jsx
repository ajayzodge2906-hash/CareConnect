...
export default function ChatbotWidget() {
  ...
  // Predefined keyword-matching logic for bot responses
  const getBotResponse = (query) => {
    const cleanQuery = query.toLowerCase().trim();

    // 1. Direct match on question title
  ...
  return (
    <div className="fixed bottom-6 right-6 z-55 flex flex-col items-end font-sans">
      ...
      {/* Chat window - Slide-in and Fade-in animation */}
      {isOpen && (
            ...
            <div className="flex flex-wrap gap-1 px-1.5 pb-1">
              {FAQS.map((faq, idx) => (
                <button
                  ...
                  className="rounded-full border border-primary-200 bg-primary-50/50 px-2.5 py-1 text-[10px] font-semibold text-primary-750 hover:bg-primary-100 hover:border-primary-300 transition-colors"
                >
                  {faq.q}
                </button>
              ))}
          ...
          <div className="border-t border-slate-150 bg-white p-3 shrink-0">
            ...
            >
              <input
                ...
                className="flex-grow rounded-lg border border-slate-350 px-3 py-1.5 text-xs font-medium text-slate-850 shadow-inner focus:border-primary-500 focus:outline-none focus:ring-1 focus:ring-primary-500"
              />
              <button
                type="submit"
